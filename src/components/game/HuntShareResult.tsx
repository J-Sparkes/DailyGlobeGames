...
function HuntShareCardPreview({ result }: { result: CompletedHuntResult }) {
  return (
    <div className="share-card-pulse rounded-xl border border-[var(--ui-border-subtle)] bg-[var(--ui-surface-raised)] p-3">
      <p className="font-stat text-[10px] uppercase tracking-wider text-[var(--ui-text-muted)]">
        Daily Globe Games · Hunt · {result.date}
      </p>
      <p className="font-stat mt-1 text-3xl font-semibold text-[var(--ui-accent-warm)]">
        {result.score}
      </p>
      <p className="text-xs text-[var(--ui-text-muted)]">/ {MAX_HUNT_SCORE} points</p>
...
export function HuntShareResult({
  result,
  rewardPop = false,
  compact = false,
}: HuntShareResultProps) {
  ...
  return (
    ...
    >
      ...
      {!compact && <HuntShareCardPreview result={result} />}
